...
import { FaMicrosoft, FaGoogle, FaApple, FaGithub, FaAmazon } from "react-icons/fa";
  ...
  return (
    ...
    <section className="flex flex-col items-center text-white text-sm pb-20 px-4 font-poppins">
      ...
      <a href="" className="flex items-center gap-2 border border-slate-700 rounded-full p-1 pr-3 text-sm mt-20">
        <span className="bg-indigo-500 text-xs px-3 py-1 rounded-full">NEW</span>
      ...
      <div className="flex flex-wrap items-center justify-center gap-16 md:gap-20 mx-auto mt-16 text-gray-400">
        {companies.map((company, index) => {
          const Icon = company.icon;
          return (
            <div
              key={index}
              className="text-5xl hover:text-white transition duration-300 hover:scale-110"
              title={company.name}
            >
              <Icon />
            </div>
